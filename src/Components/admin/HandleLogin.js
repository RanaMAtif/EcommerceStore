...
export const HandleLogin = () => {
    ...
    async function fetchOldImageUrl() {
      try {
        const docSnapshot = await getDoc(doc(fs, "LoginLogo", "Image"));
    ...
    const handleImageUpload = async () => {
      if (image) {
        const storageRef = ref(storage, "login-logo/" + image.name);
        await uploadBytes(storageRef, image);
        const downloadURL = await getDownloadURL(storageRef);
  
        // Save the downloadURL to Firestore
        const footerSettingsRef = doc(fs, "LoginLogo", "Image");
        await setDoc(
          footerSettingsRef,
    ...
    return (
      <Container>
        <div>
          <Typography variant="h6">Edit Login Page Logo</Typography>
