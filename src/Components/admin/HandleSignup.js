...
export const HandleSignup = () => {
  ...
  async function fetchOldImageUrl() {
    try {
      const docSnapshot = await getDoc(doc(fs, "SignupLogo", "Image"));
  ...
  const handleImageUpload = async () => {
    if (image) {
      const storageRef = ref(storage, "signup-logo/" + image.name);
      await uploadBytes(storageRef, image);
      const downloadURL = await getDownloadURL(storageRef);

      // Save the downloadURL to Firestore
      const footerSettingsRef = doc(fs, "SignupLogo", "Image");
      await setDoc(
        footerSettingsRef,
      ...
      toast.success("Sign Up Logo updated successfully", {
  ...
  return (
    <Container>
      <div>
        <Typography variant="h6">Edit Signup Page Logo</Typography>
